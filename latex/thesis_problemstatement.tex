...
weighted combination of scaled monthly ramp impact and regional peak impact while meeting
...
Rather than track individual jobs and their execution, we use the aggregate CPU utilization (for each five-minute window) as our demand.\footnote{Tirmazi
\emph{et al.}, in \emph{Borg: The Next Generation} \cite{tirmazi2020borg},
summarize this trace the same way, plotting each priority tier's CPU usage as
a fraction of cell capacity over hour-long intervals (their Fig.~2).
Radovanovi\'{c} \emph{et al.}, in \emph{Carbon-Aware Computing for
Datacenters} \cite{radovanovic2021carbonaware}, schedule aggregate flexible
and inflexible CPU usage per cluster, from Google's internal fleet data rather
than this trace. Naug \emph{et al.}, in \emph{SustainDC: Benchmarking for
Sustainable Data Center Control} \cite{naug2024sustaindc}, drive their
reinforcement-learning workload environment with utilization curves from
Alibaba and Google traces.}
...
We retain only top-level instances and full five-minute measurement
windows.\footnote{Using only top-level instances avoids counting both an
allocation set and the work running inside it. Google reports each instance's
usage over windows of at most 300 seconds
\cite{googleTraceFormatV3}. Following the instance-usage query example in Google's \cite{googleClusterAnalysisColab}., we keep rows whose
\codepath{end_time-start_time} is at least 300 seconds, so each retained
\(a_r\) is an average over a whole window. An audit of cells \(a\)--\(d\) found that
the 300-second filter removes 6.5\% of CPU work, proportionally more batch
(11--15\% of batch work) than service (2--8\%). Hour by hour, the retained
curves correlate with all-row curves at 0.96--0.99 in level and 0.88--0.97 in
one-hour ramps, with slightly larger relative ramps (2.6--4.9\%).}
...
\codepath{collection_id}, and join that to the usage rows, using BigQuery. }
...
include data-center-floor cooling.\footnote{See \emph{Google Data Center
Power Trace}, p.~2, under \codepath{measured_power_util}
\cite{googlePowerTraceDocumentation}.}
We use CPU utilization to predict measured power-domain utilization,
averaging both series by cell and hour before fitting an ordinary
least-squares line:
\begin{equation}
  \widehat p_c(u)
  =
...
policy \(\mu\), its normalized adjusted and original peaks in month \(e\) are
\begin{align}
...
region's demand scale, \(S_m\). As with ramps, we measure the fleet's peak
impact by subtracting the original grid's peak:
\begin{equation}
  \Phi_{m,e}(\mu)
  =
...
\begin{itemize}
  \item \codepath{extract_tier_curves.ipynb} runs the BigQuery queries in
        Google Colab. It needs a Google Cloud project and the BigQuery client
        listed in \codepath{requirements.txt}.
  ...
  \item The notebook writes \codepath{data/cells/cell_a_tiers.csv} through
        ...
        \codepath{batch_share} column. The files for cells \(e\)--\(h\) come
        from \codepath{extract_cells_eh.ipynb} and are unused.
\end{itemize}

\subsection{Power model (Section~\ref{sec:powerdata2019})}
\begin{itemize}
  \item The power-extraction cell of
        \codepath{extract_clusterdata2019_full.ipynb} averages
        ...
        \codepath{numpy.linalg.lstsq}. The notebook's machine, job, and
        workload-generator outputs are unused.
...
\begin{itemize}
  ...
  \item In \codepath{scripts/build_causal_forecasts.py},
        \codepath{build_features} lists every input, including trailing 24-
        and 168-hour means, the 25-hour lag, and calendar terms.
        \codepath{assert_causal} checks that no input uses a later row.
        \codepath{fit_ridge} solves ridge regression in closed form, with the
        penalty chosen by rolling-origin cross-validation in
        \codepath{select_alpha}. Rows without a week of history use
        persistence, as would any model that failed to beat persistence on
        training hours; none does. \codepath{--write} rewrites the panels'
        forecast columns, and \codepath{--report} prints accuracy only.
