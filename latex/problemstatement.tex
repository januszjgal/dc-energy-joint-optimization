...
The thesis has four essential parts:

\begin{enumerate}
  \item \textbf{A workload.} Measured five-minute CPU demand from four Google        Borg cells (Section~\ref{sec:clusterdata2019}).
  \item \textbf{A power model.} A fitted relationship, per cell, converting        compute utilization into a fraction of the site's rated power (Section~\ref{sec:powerdata2019}).
  \item \textbf{Four grids.} Hourly energy demand, wind, solar, and day-ahead price for four regions (Section~\ref{sec:energy-data}).
  \item \textbf{A scheduler.} A PPO agent and decoder that outputs an optimization for assignment of cpu demand to minimize grid ramp (Section~\ref{sec:problem-statement}).
\end{enumerate}
...
A 'Borg' is a cluster of a few thousand machines that jobs are submitted to in a Google Data Center. Google provides a detailed record of 8 such cells, labeled \(a\)--\(h\), from a May 2019 data capture \cite{wilkes2020clusterdata2019}. It cell contains machine capacity, per-instance resource usage, job submission events, priorities, and resource requests. Every record is also sanitized, meaning no user/application/geography/etc. is given. 

The full dataset is 2.4 TiB, so we leverage BigQuery to query the data and produce tables with the relevant information to be consumed downstream. Three BigQuery tables produce the curves this experiment consumes --- \codepath{machine_events} for capacity, \codepath{instance_usage} for measured CPU, and \codepath{collection_events} for priority. Once the curves are extracted, nothing downstream touches BigQuery again. Table~\ref{tab:clusterdata-upstream-fields} in Appendix~\ref{app:source-tables} gives the exact fields.

\subsection{Measured utilization extraction}
\label{subsec:clusterdata-utilization}

In the context of the dataset, Google normalizes CPUs so that the largest machine capacity is 1.0. In our data, we use the \codepath{average_usage.cpus} table, which is a rate that defines how much compute was consumed. 

The denominator is total cell capacity. A machine can appear several times in \codepath{machine_events}, through add, update, and snapshot records, so we take each machine's largest reported capacity and sum across machines. This picks the largest \emph{capacity} record, not the busiest moment.

The numerator sums \codepath{average_usage.cpus} across instances in each five-minute bucket, after two filters:
...
The output is one file per cell, \codepath{data/cells/cell_X_tiers.csv}, with columns \codepath{timestep}, \codepath{cpu_demand_norm}, \codepath{batch_share}, \codepath{service_demand_norm}, and \codepath{batch_demand_norm}.
...
The experiment relies on both shifting compute usage to other location, and shifting it to other temporally. To understand which demand can be shfited temporally, we use the job priority details from Google's trace. 

Every submitted job carries a priority, which Google groups into various bands. The two lowest bands (``free'' and ``best-effort batch,'' together everything at priority 115 or below) carry no service-level objective: Borg may run this work whenever there is room, and may evict it when something more important needs the machine \cite{googleClusterAnalysisColab}. For our experiment, we refer to this as batch demand. All other priorities are seen as service demand and must be served on arrival.

Priority lives in \codepath{collection_events}, while usage lives in \codepath{instance_usage}, so the extraction builds one priority record per submitted job and joins it to usage by \codepath{collection_id}. Within each five-minute bucket, batch CPU is the usage matched to a priority at or below 115, normalized by the same cell capacity as before. Service CPU is aggregate minus batch, so the two components add back to the measured total at every timestep. The output of this extraction is two demand curves - the \codepath{service_demand_norm} and \codepath{batch_demand_norm}, for every 5 minute bucket. Note it may be cleaner to refer to these as usage/consumption curves, but we are quantifying them as demand on the datacenter.
...
The previous section produced a csv with details on how busy each cell was. To convert this to energy that the datacenter demand required, we can use PowerData2019, also provided by Google as part of the Cluster Data. PowerData2019 provides 5 minute, per cell, power utilization for the server equimpent. It is also normalized, which is why we later state an assumption on the total power rating of a given datacenter.

We take the average of \codepath{measured_power_util} in a given hour, and pair this with average CPU utilization for the same hour. Each cell is then fitted with an orindary least squares line, allowing us to define a cell's power usage by it's noramlized CPU utilization. This can be writen as:
\begin{equation}
  \widehat{p}_{c}(u)
  =
  \alpha_c + \beta_c u,
...
Four markets are used for energy data: CAISO NP15 (northern California), MISO Minnesota, SPP North, and ISO-NE NEMA (greater Boston). All four physical series come from the same EIA schema. This data is pulled from September 2025 to Februrary 2026. We intentionally use the most recent grid data to align with how demand shifting would impact todays grids, rather than 2019.

More than 31 days are required to allow the agent iteself to have seperate training and validation data.
...
Hourly net load is computed once:
\begin{equation}
  N_{m,t}
  =
...
Two small files define the energy setup. \codepath{data/four_market_v2/calendar.json} lists the 114 training days and 28 validation days, and \codepath{data/four_market_v2/frozen_stats.json} holds the training-only constants.
...
To be clear, the cell to market mapping is arbitrary. Google doesn't provide geographic information on their data centers (as explained prior, everything is sanitized and normalized). 

Similarly, we gave each site a 500 MW rating as an assumption. This is reasonable given the sizing of large hyperscaler facilities, and provides enough of a total fleet power to produce a noticable change in grid demand. This is also easily adaptable, if for example, we wanted to test the grid impact on all data center energy demand.
...
Two items are apparent when looking at this equation. First, because \(\alpha_c\) is around 0.4--0.55, an idle site still draws 190--275 MW; the scheduler can move a few hundred megawatts around, not two gigawatts. Second, because \(\alpha_c+\beta_c<1\) in every fitted cell, a fully loaded site draws roughly 434, 462, 483, and 476 MW for cells \(a\)--\(d\) rather than the full 500.
...
Using the previous section outputs: hourly service and batch arrivals \(s_{m,t}\) and \(q_{m,t}\), the deferral windows \(H_m\), site capacity \(K_m\) and rated power, the power-model coefficients, the four grids' net load \(N_{m,t}\) and day-ahead price \(L^{\mathrm{DA}}_{m,t}\).
...
Write \(x^{\mathrm{svc}}_{m,t}\) and \(x^{\mathrm{bat}}_{m,t}\) for the service and batch work executed at site \(m\), and \(z_t=\sum_m x^{\mathrm{bat}}_{m,t}\) for total batch execution that hour.
...
The three warm hours exist so the first decision can be scored: a one-hour ramp needs an hour to compare against, and a three-hour ramp needs three. The tail exists for the mirror-image reason, and for one more --- it gives batch work deferred late in the day somewhere to land, and it lets ramp windows that end after the last arrival still be counted. The day never wraps around to its own start. The queue begins empty and must end empty.
...
\begin{enumerate}
  ...
  \item the fleet's added power reduces the native grid's net-load changes across the four market cases.
\end{enumerate}
\end{quote}
