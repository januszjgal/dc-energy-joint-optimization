"""Frozen energy-model v2 two-fold OOF PPO campaign across cells a-h.
...
* Every environment retains four equal 100 MW proxy DCs and 8,928 steps.
...
Usage:
    ...
    python scripts/run_oof_campaign_v2.py --phase all --workers 16
"""
...
PROTOCOL_CONFIG = load_protocol()
ALPHA = float(PROTOCOL_CONFIG["objective"]["peak_penalty_weight"])
TIMESTEPS = int(PROTOCOL_CONFIG["oof"]["training_timesteps"])
SEEDS = list(PROTOCOL_CONFIG["oof"]["seeds"])
SERVICE_COMPLETION_FLOOR = 0.9999
BATCH_COMPLETION_FLOOR = 0.9999
COMPLETION_PENALTY = float(
    PROTOCOL_CONFIG["objective"]["completion_penalty_weight"]
)
...
MODEL_ROOT = ROOT / "models" / "oof_v2_2025"
LOG_ROOT = ROOT / "logs" / "oof_v2_2025"
OUT_ROOT = ROOT / "output" / "oof_v2_2025"
...
SOURCE_FILES = [
    ...
    "scripts/run_oof_campaign_v2.py",
    "requirements.txt",
...
def protocol() -> dict[str, Any]:
    ...
    return {
        "protocol": "energy-model-v2-2025-cross-cell-two-fold",
        "protocol_config": str(
            V2_2025_PROTOCOL.relative_to(ROOT)
        ).replace("\\", "/"),
        "protocol_config_sha256": sha256(V2_2025_PROTOCOL),
        ...
        "objective": PROTOCOL_CONFIG["objective"],
        "routing_assumption": PROTOCOL_CONFIG["routing"],
        "ramp_evaluation": PROTOCOL_CONFIG["ramp"],
        "attribution": PROTOCOL_CONFIG["attribution"],
        "test_scope": "full 8,928-step month on opposite four-cell fold",
        "headline_metrics": [
            "held_out_total_cost",
            ...
            "physical_ramp_metrics_1h",
            "physical_ramp_metrics_3h",
        ],
        "success_rule": {
            "savings": (
...
def write_protocol() -> None:
    ...
    if path.exists():
        ...
        if immutable != current:
            raise RuntimeError(
                "output/oof_v2_2025/protocol.json differs from the current "
                "protocol; "
...
def assert_clean_training_source(python: Path) -> None:
    ...
    if tracked_diff.returncode != 0:
        raise RuntimeError(
            "tracked source differs from HEAD; commit the frozen v2 protocol "
            "before training"
        )
    subprocess.run(
        [str(python), "scripts/preflight_energy_model_v2.py"],
        cwd=ROOT,
        check=True,
...
def train_one(
    ...
    cmd = [
        ...
        str(timesteps),
        "--lr",
        str(PROTOCOL_CONFIG["oof"]["ppo"]["learning_rate"]),
        "--n-steps",
        str(PROTOCOL_CONFIG["oof"]["ppo"]["n_steps"]),
        "--batch-size",
        str(PROTOCOL_CONFIG["oof"]["ppo"]["batch_size"]),
        "--net-arch",
        *[
            str(width)
            for width in PROTOCOL_CONFIG["oof"]["ppo"]["net_arch"]
        ],
        "--peak-penalty-weight",
        str(ALPHA),
        "--seed",
...
def main() -> None:
    ...
    parser.add_argument(
        "--phase",
        choices=["preflight", "train", "evaluate", "all"],
        default="all",
    )
    parser.add_argument(
    ...
    assert_clean_training_source(args.python)
    write_protocol()
    assert_frozen_provenance()
    if args.phase == "preflight":
        print(
            f"Frozen v2 protocol ready: {OUT_ROOT / 'protocol.json'}",
            flush=True,
        )
        return
