...
\begin{table}[htbp]
  \centering
  \footnotesize
  \caption{Which Borg cell supplies each site's workload, and the length
  \(H_m\) of its batch execution window in hourly slots, counting the arrival
  slot. A window of 1 allows no delay; a window of 3 allows at most two hours.
  Each site is named by its market case throughout.}
  \label{tab:active-market-map}
  \begin{tabular}{@{}llr@{}}
    \toprule
    Market case & Source cell & \(H_m\) (slots) \\
    \midrule
    CAISO NP15     & \(a\) & 2 \\
    MISO Minnesota & \(b\) & 1 \\
    SPP North      & \(c\) & 2 \\
...
10,837 MW, and for MISO a forecast of 78,530 MW against an eventual
...
\end{align}
All queues begin empty at the first hour of the month. How much temporal flexibility the problem contains is
...
\end{equation}
The minimum only bites in the last two hours of a month: work arriving there
loses part of its window so that the month can close with empty queues, an
edge effect touching at most two of the 744 hours of May.
...
queue.\footnote{When two parcels of work share a deadline, the one from the
lower-numbered origin is drained first, and within an origin the earlier
arrival goes first. The rule is a total order, so the same request always
drains the same work.} At the end of the month,
\begin{equation}
  B_{i,T_e}=0,
...
\begin{itemize}
  ...
  \item the gross-demand and net-load forecasts issued at hour \(t-1\),
        which cover hours \(t\), \(t+1\), and \(t+2\);
