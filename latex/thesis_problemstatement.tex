...
equally. We measure the fleet's impact by subtracting the original grid's
squared ramp:
\begin{equation}
  I_{m,t}
  =
...
to the original grid; a positive value means it increased it. The objective
...
relative to the original grid; a positive \(J\) means it increased it.
...
The \emph{original grid}, with \(P_{m,t}=0\), provides the reference
\(\Delta^{\mathrm{original}}_{m,t}\) in Equation~\eqref{eq:incremental-impact}. We compare
