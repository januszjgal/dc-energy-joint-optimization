...
regional net-load ramps and reduce regional monthly net-load peaks relative
to the same fleet without flexibility, using temporal deferral and geographic
...
weighted combination of normalized monthly ramp and peak scores while meeting
workload deadlines and site-capacity limits.

We focus on net load rather than cost: rapid changes require generation to
adjust quickly, while high peaks require more supply at once.
...
equally. As a diagnostic, we measure the fleet's incremental ramp impact by
...
\(I_{m,t}\) is not the joint objective.

Each simulated month \(e\) is one optimization episode. For scheduling policy
\(\mu\), define the mean hourly fleet ramp score and the sum of regional
normalized monthly net-load peaks:
\begin{align}
  R_e(\mu)
  &=
  \frac{1}{T_e}
  \sum_{t\in\mathcal T}\sum_{m\in\mathcal M}
  \left(\Delta^{\mathrm{adjusted}}_{m,t}(\mu)\right)^2,
  \label{eq:monthly-ramp-score}\\
  Q_e(\mu)
  &=
  \sum_{m\in\mathcal M}
  \max_{t\in\mathcal T}\frac{A_{m,t}(\mu)}{S_m}.
  \label{eq:monthly-peak-score}
\end{align}
The warm-up hour contributes to the first ramp but never to a monthly peak.
Each region has its own peak maximum; \(Q_e\) is not the maximum of the
combined regions, a data-center power peak, or an hourly squared-load penalty.
One hour defines the ramp measurement, not the planning horizon.

\subsection{Joint objective and fixed reference scales}
\label{subsec:problem-normalization}

Let \(\mathcal E_{\mathrm{train}}\) be the eleven training months and
\(\mu_{\mathrm{NF}}\) the no-flexibility policy defined below. We take the
arithmetic mean of its monthly scores:
\begin{align}
  C_R
  &=\frac{1}{11}\sum_{e\in\mathcal E_{\mathrm{train}}}R_e(\mu_{\mathrm{NF}}),
  \label{eq:ramp-reference-scale}\\
  C_Q
  &=\frac{1}{11}\sum_{e\in\mathcal E_{\mathrm{train}}}Q_e(\mu_{\mathrm{NF}}).
  \label{eq:peak-reference-scale}
\end{align}
For the current data, \(C_R\approx0.00826852\) and \(C_Q\approx3.66460\).
These positive scales are frozen for all training and May validation.
They use absolute adjusted-grid scores, never the signed incremental
diagnostic \(I_{m,t}\).

Let \(\mathfrak M\) be the causal scheduling policies satisfying
Equations~\eqref{eq:service-conservation}--\eqref{eq:site-capacity}. The joint
objective uses equal weights by default:
\begin{equation}
  \begin{aligned}
  ...
  \label{eq:objective}
\end{equation}
Equal weights give the same value to equal proportional changes relative to
these fixed training references, not equal achieved savings or learned
influence. A lower joint score need not improve both components.
Comparison variants can replace the two weights with nonnegative
\(\lambda_r,\lambda_p\), where \(\lambda_r+\lambda_p=1\); ramp-only and
peak-only policies still use the same no-flexibility baseline.

\subsection{Reference signal and the one baseline}
\label{subsec:problem-baselines}

The \emph{original grid}, with \(P_{m,t}=0\), provides the diagnostic reference
\(\Delta^{\mathrm{original}}_{m,t}\) in Equation~\eqref{eq:incremental-impact}.
It is not another scheduling baseline, and \(J_e\) is not a signed comparison
with it. We compare scheduling policies using the same added fleet.
...
The no-flexibility baseline is feasible and is currently the only scheduling
comparison used in this thesis.

We also report raw \(R_e\) and \(Q_e\), incremental ramp diagnostics
\(I_{m,t}\), and regional adjusted peaks in MW with their reductions from
the same no-flexibility fleet. Since site power is nonnegative, peak reduction
...
volumes, and batch destinations to minimize the joint monthly ramp and
...
PPO is an approximate solution method, not a guarantee of global optimality.
The raw, undiscounted monthly reward sums to \(-J_e\); training uses a
discount factor of 0.99 and reward normalization. The methods chapter will
describe the reward construction and training procedure.

\subsection{Worked example: one-hour ramp diagnostic}
\label{subsec:problem-worked-ramp}
...
Table~\ref{tab:incremental-ramp-example} shows the baseline's one-hour ramp
diagnostic.

\begin{table}[htbp]
  \centering
  \footnotesize
  \caption{Incremental ramp diagnostic from Equation~\eqref{eq:incremental-impact} at
  ...
  \label{tab:incremental-ramp-example}
  \begin{tabular}{@{}lrrr@{}}
    \toprule
    Market case & \(\Delta^{\mathrm{original}}\) & \(\Delta^{\mathrm{adjusted}}\) & \(I\ (\times10^{-5})\) \\
    \midrule
...
Let's review ISO-NE NEMA closely. Its original ramp is \(+0.0216\); increasing
...
hour, not necessarily the joint monthly objective. The example does not
establish a monthly peak reduction; evaluating \(J_e\) requires the full
month's adjusted-load trajectory, including effects on later ramps.
