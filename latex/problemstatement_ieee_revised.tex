...
Table~\ref{tab:energy-sources} lists which authority stands in for which
market case.
...
\begin{table*}[t]
  ...
  \toprule
  Market case & Physical demand and renewables \\
  \midrule
  CAISO NP15     & EIA CISO balancing-authority demand, wind, and solar
                   \cite{eiaGridMonitor}. \\
  MISO Minnesota & EIA MISO balancing-authority demand, wind, and solar. \\
  SPP North      & EIA SWPP balancing-authority demand, wind, and solar. \\
  ISO-NE NEMA    & EIA ISNE balancing-authority demand, wind, and solar. \\
  \bottomrule
  \end{tabular}
\end{table*}
...
scheduler, giving 334 UTC days, and May is held back to test it on, giving
31. May was chosen rather than a month at either end of the year because its
long days and mild temperatures give California the deepest midday solar
trough of the year, which makes it the hardest month for the thing we are
trying to reduce.\footnote{With warm and tail support, the retained panels
run from 2024-12-31 at 21:00 UTC to 2026-01-01 at 02:00 UTC.}
...
The panel is stored as one file per simulated day, each holding the thirty
hours that day needs: the three warm hours before it, its own twenty-four,
and the three-hour tail. There are 365 of them, one for every day of 2025.
Each file is grid data only. The workload for that day is attached separately
by the month-reset rule of Section~ef{subsec:transform-pairing}, which is
what lets the same grid day be paired with a different slice of the trace
...
\begin{table}[htbp]
  \centering
  \footnotesize
  \caption{Arrivals for the day beginning 2025-07-19 00:00 UTC, in work
  ...
  \begin{tabular}{@{}rrrrrr@{}}
    ...
    \midrule
     0 & 432 & 0.4225 & 0.1267 & 0.4248 & 0.1184 \\
     1 & 433 & 0.4351 & 0.1034 & 0.3958 & 0.1237 \\
     2 & 434 & 0.4233 & 0.0620 & 0.3876 & 0.1191 \\
    \multicolumn{6}{c}{\(\vdots\)} \\
    16 & 448 & 0.3982 & 0.0708 & 0.5521 & 0.0199 \\
    \multicolumn{6}{c}{\(\vdots\)} \\
    23 & 455 & 0.4289 & 0.0688 & 0.4014 & 0.1215 \\
    24 & 456 & 0      & 0      & 0      & 0      \\
    25 & 457 & 0      & 0      & 0      & 0      \\
    26 & 458 & 0      & 0      & 0      & 0      \\
    \bottomrule
...
The grid series are read off at those same hours:
\begin{equation}
  \begin{aligned}
...
\begin{table*}[t]
  ...
  \begin{tabular}{@{}L{0.16\textwidth}L{0.32\textwidth}L{0.44\textwidth}@{}}
    ...
    \midrule
    Observed
      & CPU execution, priority, normalized power, grid demand, wind, and
        solar
...
Moving 0.15 work units of Minnesota's service to CAISO NP15 would drop
Minnesota's power to 334 MW and raise NP15's to 387 MW. Minnesota sheds 28 MW
into a market that is climbing, and NP15 picks up 25 MW into a market that is
falling.\footnote{The two numbers differ because the sites have different
power slopes \(\beta_m\); moving work between sites changes total fleet
power slightly, in this case by \(-3\) MW.}
...
simulated, and late batch work must still meet its deadline there.
...
regional net-load trajectories, and causal forecast information, construct a
causal hourly schedule for service destination, batch
...
\(I=+4.03\times10^{-5}\) --- four times the size of anything the other
three markets contribute, from the smallest physical ramp on the page. This
...
service out of Minnesota and into CAISO NP15. NP15 now rises to 387 MW
instead of falling to 361, which partly fills the hole its region's net load
is digging as the morning solar climbs, and its impact falls from
\(-0.05\times10^{-5}\) to \(-9.31\times10^{-5}\). Minnesota, whose net
load is climbing, sheds 28 MW and improves too, from
\(+0.09\times10^{-5}\) to \(-1.45\times10^{-5}\). Across all four
markets the hour moves from \(+0.96\times10^{-5}\) to
\(-1.74\times10^{-5}\).
...
\begin{longtable}{@{}L{0.16\textwidth}L{0.50\textwidth}L{0.26\textwidth}@{}}
  ...
  \eqref{eq:episode-grid-map}
    & Maps absolute grid rows to episode slots
    & Ramp inputs \\
