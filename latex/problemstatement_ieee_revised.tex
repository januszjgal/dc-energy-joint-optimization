...
\begin{quote}
\emph{How much can a
causal scheduler reduce normalized one-hour regional net-load ramps over a
full month by jointly using temporal deferral and geographic routing, while
...
proximal policy optimization (PPO) to train a policy that minimizes
total one-hour ramp impact across all markets and hours of each month while
meeting workload deadlines and site-capacity limits.

We focus on net-load ramps (rather than cost) because rapid changes in net load require
generation to adjust quickly.
...
\codepath{gross_demand_mw}, \codepath{wind_mw}, and \codepath{solar_mw}.} All series use the UTC timestamps reported by EIA, giving one row per market per hour.
...
We use 2025 grid data: eleven months (334 UTC days) for training and May
(31 UTC days) for validation. Each month is a continuous simulation, as
described in Section~\ref{subsec:transform-pairing}.

To compare ramps across markets of different sizes, we define a fixed scale
...
and validation. We use them to express ramps as fractions of each market's
demand scale.
...
decision hours are \(\mathcal T=\{0,\ldots,T_e-1\}\), where \(T_e\) is the
month's length in hours. One preceding hour,
...
that warm-up work is not added to the queues.
...
be the hour within the month and \(h\) the corresponding trace hour:
\begin{align}
  s_{m,t}
  &=
  \frac{K_m\Delta t}{12}
...
\end{align}
The trace restarts each month, so \(h=t\). With \(K_m=1\) work unit per hour
...
native ramps over the 334 training days. These are raw MW changes; the
objective later divides by \(S_m\Delta t\) to express them as fractions of
each market's demand scale per hour.
Against the median one-hour native ramp, a site's idle-to-full power range
is 19\% in CAISO NP15,
...
\begin{table}[htbp]
  \centering
  \footnotesize
  \caption{Absolute one-hour native net-load ramp magnitudes over training
  hours, in MW.}
...
month with \(T_e\) hours, and \(\mathcal W=\{-1\}\) the preceding warm-up
slot. One interval has duration \(\Delta t=1\) h.
...
\end{equation}
Releases follow earliest-deadline order and are increased when necessary to
...
\end{align}
A 1,000 MW change represents a smaller fraction of MISO's demand scale than
of ISO-NE's.

Squaring penalizes larger ramps and treats upward and downward ramps
equally. We measure the fleet's impact by subtracting the native grid's
squared ramp:
\begin{equation}
  I_{m,t}
  =
  \Delta_{m,t}^{\,2}
  -
  \left(\Delta^0_{m,t}\right)^2.
  \label{eq:incremental-impact}
\end{equation}
A negative value means the scheduled fleet reduced the squared ramp relative
to the native grid; a positive value means it increased it. The objective
does not require improvement in every market and hour. We write
\(I_{m,t}(\mu)\) when showing its dependence on scheduling policy \(\mu\)
through \(P_{m,t}\).

Each simulated month is one optimization episode. The objective adds the
one-hour ramp impact for every market and every hour of that month.
One hour defines the ramp measurement, not a one-hour
planning horizon. Each action uses only the information then available, as
defined in Section~\ref{subsec:problem-causality}. Let \(\mathfrak M\) be the
set of causal scheduling policies that satisfy
...
Read from the inside out, that says: add the fleet's one-hour ramp impacts
across all markets and hours of the month,
and choose the causal policy with the smallest total. A negative \(J\)
means the fleet reduced the month's total normalized squared ramp score
relative to the native grid; a positive \(J\) means it increased it.

Ramps are evaluated relative to each market's demand scale, so equal
changes in MW need not receive equal scores.

\subsection{Reference signal and the one baseline}
\label{subsec:problem-baselines}

The \emph{native grid}, with \(P_{m,t}=0\), provides the reference
\(\Delta^0_{m,t}\) in Equation~\eqref{eq:incremental-impact}. We compare
scheduling policies using the same added fleet.

The \emph{no-flexibility baseline}, \(\mu_{\mathrm{NF}}\), runs every arrival
at its original site in its arrival hour. It uses the same arrivals, power
models, grid data, forecasts, and month as the scheduler:
\begin{equation}
...
The no-flexibility baseline is feasible and is the only scheduling
comparison used in this thesis.
...
\begin{itemize}
  ...
  \item modeled site power from hour \(t-1\); and
...
volumes, and batch destinations to minimize total one-hour ramp impact
across all markets and hours of the month in Equation~\eqref{eq:objective}. Use only the
...
\subsection{Worked example: from workload to ramp impact}
\label{subsec:problem-worked-objective}

At 2025-07-19 16:00 UTC (trace hour 448), MISO Minnesota receives
0.3982 work units of service and 0.0708 of batch. Under the no-flexibility
baseline, it executes \(w=0.4690\) work units. Its modeled power is
\begin{equation*}
  P = 500\,(0.5482 + 0.3758 \times 0.4690) = 362 \ \mathrm{MW}.
\end{equation*}
CAISO NP15, SPP North, and ISO-NE NEMA draw approximately 361, 350, and
333 MW, respectively.

Table~\ref{tab:energy-example-rows} gives the grid conditions for that hour.
CAISO net load fell by 2,112 MW from the previous hour, while MISO net load
rose by 2,826 MW. The 15:00 forecasts available to the scheduler predicted
current-hour net loads of 10,840 MW for CAISO and 78,530 MW for MISO.
The calculations below use the realized values to score an illustrative
routing choice.

\begin{table}[htbp]
  \centering
  \footnotesize
  \caption{The four market cases at 2025-07-19 16:00 UTC, in MW.}
...
Table~\ref{tab:objective-example} shows the baseline's one-hour ramp impact.

\begin{table}[htbp]
  \centering
  \footnotesize
  \caption{Contribution to Equation~\eqref{eq:objective} at
...
Let's review ISO-NE NEMA closely. Its native ramp is \(+0.0216\); increasing
site power raises the adjusted ramp to \(+0.0225\), giving
\(I=+4.03\times10^{-5}\). Summing the four markets gives a baseline
total impact for this hour of \(+3.83\times10^{-5}\).

Now move Minnesota's 0.3982 units of service to CAISO NP15, which has
0.4281 units of spare capacity. Minnesota's power falls from 362 to
287 MW, while CAISO's rises from 361 to 429 MW. Their different power
coefficients mean the 75 MW reduction and 68 MW increase need not match.
This reduces demand in the region with rising net load and adds demand
in the region with falling net load.

CAISO's one-hour impact becomes \(-24.39\times10^{-5}\), and MISO's becomes
\(-3.98\times10^{-5}\). With the other two sites unchanged, the total
falls to \(-24.58\times10^{-5}\). This improves the score for this
hour; the monthly objective also accounts for the effects of this decision
on later hours.
...
A later version could expand the warm-up period to give the scheduler more
historical grid observations.
...
\begin{longtable}{@{}L{0.18\textwidth}L{0.56\textwidth}L{0.18\textwidth}@{}}
  ...
  \(\mathcal E_{c,k}\) & Retained usage rows for cell \(c\), bucket \(k\) & -- \\
  \(\mathcal E^{\mathrm{bat}}_{c,k}\) & Rows in \(\mathcal E_{c,k}\) matched to priority \(\leq115\) & -- \\
  \(\mathcal M\) & Four modeled sites and market cases & -- \\
  \(\mathcal T\) & Decision-slot indices \(0,\ldots,T_e-1\) of a month & -- \\
  \(\mathcal W\) & Preceding warm-up hour \(\{-1\}\) & -- \\
  ...
  \(\alpha_m,\beta_m\) & Power coefficients for the assigned site & fraction \\
  ...
  \(\Delta^0_{m,t}\) & Normalized one-hour native ramp & fraction/h \\
  \(\Delta_{m,t}\) & Normalized one-hour adjusted ramp & fraction/h \\
  \(I_{m,t}\) & Incremental squared one-hour ramp impact & h\(^{-2}\) \\
  \(J(\mu)\) & Total one-hour ramp impact across all markets and hours of a simulated month under policy \(\mu\) & h\(^{-2}\) \\
...
\begin{longtable}{@{}L{0.16\textwidth}L{0.50\textwidth}L{0.26\textwidth}@{}}
  ...
  \eqref{eq:batch-total}
    & Matches batch releases to execution
    & Queue and site load \\
  ...
  \eqref{eq:batch-deadline}
    & Assigns latest execution slots, limited by month-end
  ...
  \eqref{eq:native-ramp}
    & Defines normalized one-hour native ramp
    & Incremental impact \\
  \eqref{eq:adjusted-ramp}
    & Defines normalized one-hour ramp after site power
    & Incremental impact \\
  \eqref{eq:incremental-impact}
    & Compares adjusted and native squared one-hour ramps
    & Objective \\
  \eqref{eq:objective}
    & Minimizes total one-hour ramp impact across all markets and hours of the month
    & Scheduling objective \\
  \eqref{eq:no-flex-baseline}
    & Defines the no-routing, no-deferral baseline
    & Evaluation \\
